...
import { getReglamentoPdfUrl } from '@/infrastructure/pdf/reglamento'
...
export function ResourceModals({ activeResource, onClose }: Props) {
  ...
  return (
    <>
      ...
      {/* Reglamento Académico — PDF Viewer */}
      <ReglamentoModal open={activeResource === 'Reglamento Académico'} onClose={onClose} />
...
/* ─── Reglamento PDF Modal ─── */
function ReglamentoModal({ open, onClose }: { open: boolean; onClose: () => void }) {
  const pdfUrl = useMemo(() => open ? getReglamentoPdfUrl() : '', [open])
          ...
          >
            ...
            <div style={{
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                ...
                <div>
                  <div className="heading-font" style={{ fontWeight: 700, fontSize: 16, color: '#1f2d33' }}>Reglamento Académico</div>
                  <div style={{ fontSize: 11.5, color: '#9aa7ad' }}>Diploma de Especialización — Semestre 2026</div>
                </div>
              </div>
              <div style={{ display: 'flex', gap: 8 }}>
                <a href={pdfUrl} download="Reglamento_Academico_2026.pdf" style={{
